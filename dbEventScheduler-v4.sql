...
CREATE PROCEDURE RemovalOfOldData(
    IN db_name VARCHAR(255),
    IN table_name VARCHAR(255),
    IN column_date VARCHAR(255),
    IN base_date VARCHAR(255),
    IN months_before INT
)
BEGIN
    DECLARE cutoff_date DATE;

    SET cutoff_date = DATE_SUB(base_date, INTERVAL months_before MONTH);
...
CALL RemovalOfOldData('sakila', 'payment', 'payment_date', '2005-07-07', 2);

...
BEGIN
    ...
    IF EXISTS (SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = db_name) THEN
        ...
        ELSE
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Table does not exist in the specified database!';
        END IF;
    ELSE
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Specified database does not exist!';
    END IF;
END //
